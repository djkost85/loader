...
if(preg_match("/<div\s*class=\"countResult\">\s*Всего\s*найдено.\s*(\d+)\s*<\/div>/iU", $content, $match)) $count_page=ceil($match[1]/15);
else return false;
...
do {
    $reg_ex='#<tr\s*class="proxyStr">\s*<td>\s*<script\s*type="text/javascript">\s*(?<js>[^<]*)\s*</script>\s*</td>\s*<td>\s*(?<type_proxy>.*)\s*</td>#iUm';
    ...
    {
        if(!preg_match('#Proxy=String.fromCharCode\((?<js_code>[^\)]*)\)#iU', $str_secret_code,$match_secret_array)) break;
        ...
        preg_match_all('#(?:\(|\+)(?<ip>\w+)#', $str_secret_code,$matches_secret_var);
        $ip="";
        foreach ($matches_secret_var['ip'] as $key_ip => $value_ip)
            if(preg_match('#'.$value_ip.'=\'(?<ip>[^\']*)\'#', $str_secret_code, $match_ip)) $ip.=$match_ip['ip'];
    ...
    $pagenation++;
    if(!$content=$get_seprox_content->get_content($url_source.$pagenation.".html")) break;
